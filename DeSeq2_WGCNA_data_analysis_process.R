...
#PC1 and PC2
PCA_All_DEGs_figure_1 <- ggplot(PCA_All_DEGs_figure_data, aes(x=Dim.1, y=Dim.2, col = variable1, shape = variable2)) +
  geom_point( size = 3) +
...
PCA_All_DEGs_figure_2 <- ggplot(PCA_All_DEGs_figure_data, aes(x=Dim.2, y=Dim.3, col = variable1, shape = variable2)) +
  geom_point( size = 3) +
...
#####Redo PCA for general transcriptome characterization
...
##If there is a good reason, you can filter your sample again
...
#Correlation analysis of MEs and your variables of interest
...
png('Module_trait relationships.tiff', units="in", width = 4, height=8, res=600)
par(mar = c(4,6, 4, 1))
